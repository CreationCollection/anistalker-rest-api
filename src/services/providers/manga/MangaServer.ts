...
import { extractChapterData, extractMangaData, extractPanelUrl } from "./MangaInformer.js"
...
export default class MangaDex {
    ...
    {
        let url = `https://api.mangadex.org/manga/${mangaId}/feed?translatedLanguage[]=en&limit=20&offset=${page}`
        let result: AxiosResponse = await axios.get(url)
        let lastPage = parseInt(result.data.total) / 20
...
class MangaPage implements IMangaPage {
    ...
    async get(page: number): Promise<Manga[]> {
        let url = this.url.replace("@offset", page.toString())
        let result: AxiosResponse = await axios.get(url)

        let content: Manga[] = []

        this.lastPage = parseInt(result.data.total) / parseInt(result.data.limit)
